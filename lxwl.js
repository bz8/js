...
'聊城捷弘物流有限公司(371523198509130020)'
);

// 是否已注册
function registed() {
	var comName = $('#banner_left').text().split('\n')[2].trim();
	for(var index in comNames) {
		if(comNames[index] == comName) {
			return true;
		}
	}
	return false;
...
// 已注册, 修改页面
if (registed()) {
  $("#cont").prepend("&nbsp;&nbsp;&nbsp;&nbsp;<button type='button' onclick='btnSplitSubmit_Onclick()'>填写并提交</button>");
  $("#cont").prepend("&nbsp;&nbsp;&nbsp;&nbsp;<button type='button' onclick='btnSplit_Onclick()'>填　　　写</button>");
  $("#cont").prepend('<input type="checkbox" name="items" id="ckbJumpYzm" checked="checked" />跳过验证码');
  $("#cont").prepend("<textarea cols='80' rows='15' id='infos' />");
...
function removeTextBox(o) {
  ...
  if (t > 1) document.getElementById(id).removeChild(o.parentNode);
}
